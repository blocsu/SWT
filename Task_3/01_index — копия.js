...
const s = Server((req, res) => {
   if(req.url === '/') {
     ...
     return res.end(`${s}`);
      ...
      const p = async () => {
        let src = 'https://bloc.su/';
        wFS('input.txt', `${src}`);
        let valueInput = cRS('input.txt').pipe(res);
        res.writeHead(200, {'Content-Type': 'text/html; charset=utf-8', ...CORS});
        const tagInput = `<input id="inp" type="url" value=${valueInput}>`;
        const tagButton = '<button id="bt">Click</button>';
        res.write(`${tagInput}`);
        res.write(`${tagButton}`);        
        src = await fetch(cRS('input.txt')).then(x => x.text());
      
        //return res.end(`${tagInput.value}`);
      }
      p();
